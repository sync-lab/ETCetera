...
class IncorrectNumOfSymbolicExpressionException(GenericParsingException):
    ...
    def __init__(self, expr, found, exp):
        base_msg = f'Invalid Number of Symbolic Expressions in {expr}. Expected {exp}, Found: {found}. '
        super().__init__(base_msg)

class IncorrectDynamicsException(GenericParsingException):
    def __init__(self, found, exp):
        base_msg = f'Invalid Dynamics Found. Expected {exp}, Found: {found}. '
        super().__init__(base_msg)

class IncorrectNumberOfVariablesSpecifiedException(GenericParsingException):
    def __init__(self, var, found, exp):
        base_msg = f'Incorrect number of {var} variables found. Expected {exp}, Found: {found}. '
        super().__init__(base_msg)