...
from ETCetera.exceptions.parser_exceptions.vector_matrix_syntax_exceptions import \
...
def check_matrix_syntax(matrix_str):
    ...
    for row in matrix_str_searched.split(';'):
        temp = len(list(filter(None, row.split(' '))))
        if (row_len == 0 or temp == row_len):  # Check if first row or number of digits is same as previous row
            row_len = temp
            # row_len = len(list(filter(None, row.split(' '))))   # Get length of split string
        else:
            raise IncorrectMatrixDefinitionException(row, row_len, temp)
...
def check_symbols_in_exprs(allowed_chars, sym_expr):
    ...
    ----------
        ...
        line_num : int
            An integer specifying the line in the input file.
    ...
    for item in re.findall(r'\w+', sym_expr):
        # TODO: Raise exception when func not in list_sympy_funcs?
        temp = re.split('(\d.*)', item)
        if not item.isdigit() and not temp[0] in list_sympy_funcs:  # Substring should not be number or sympy function
            if len(temp) == 1 or temp[0] not in allowed_chars:    # If substring is like 'x' or not in allowed chars
                raise ArbitraryVariableNamingException(temp[0], allowed_chars)
            elif temp[1] == '0':
