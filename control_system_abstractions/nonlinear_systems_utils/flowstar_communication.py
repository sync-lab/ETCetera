...
def write_model_file(init_box, goal_symbolic, time_max, symbol_list, dynamics, parameter_list, parameter_box, remainder, file_path,file_name="file.model",verbose=True):
    ...
    with open(os.path.join(file_path,file_name),'w+') as f:
        f.write(text)
    if (verbose ==True):    
        print("Model File exported at " + os.path.join(file_path,file_name))
    return 
        
def call_flowstar(flowstar_path,file_path,file_name ="file.model", verbose = True, time_out = None):    
    # Check file suffix
    if not file_name.endswith('.model'):
        file_name = file_name + '.model'
    #Initialize results
    result ={}
    if (verbose ==True):
        print("Calling flowstar")
    result['time-out']= False
    try:
        word = flowstar_path + ' < ' + file_path + '/'+file_name
        if (time_out == None):
        ...
        if verbose:
            print("flowstar time-out or other unexpected result.")
    ...
    if ('is not large enough' in output_flowstar):
        ...
        result['sat'] = True
        if verbose:
            print("flowstar remainder is not large enough. Terminate the program and enlarge the remainder.")        
    return result    

def flowstar_verify(init_box, goal_symbolic, time_max, symbol_list, dynamics, parameter_list, parameter_box,\
                  flowstar_path,file_path,file_name ="file.model",verbose = True, time_out = None, remainder = 1e-1,):
    # Check file suffix
    if not file_name.endswith('.model'):
        file_name = file_name + '.model'
    
    write_model_file(init_box, goal_symbolic, time_max, symbol_list, dynamics, parameter_list, parameter_box, remainder, file_path,file_name,verbose)
    
    result = call_flowstar(flowstar_path,file_path,file_name, verbose, time_out)
    return result
